...
def heading_adjustment_from_buoy(offset_cm, distance_cm, buoy_type, safe_margin_cm=100):
    if buoy_type in ['red', 'west'] and offset_cm >= 0:
        print(f"[DECISION] {buoy_type} buoy detected on the RIGHT. Correcting heading to the RIGHT.")
        ...
        print(f"[DECISION] {buoy_type} buoy detected on the LEFT. Correcting heading to the LEFT.")
        corrected_offset = offset_cm - safe_margin_cm
        return math.degrees(math.atan2(corrected_offset, distance_cm))
    else:
        print(f"[DECISION] {buoy_type} buoy is already on the correct side. Continuing toward waypoint.")
        return 0.0
...
def apply_cv_navigation(vessel, buoy_detections, frame_dims, fov, buoy_real_height_cm=60, safe_margin_cm=100):
    ...
    print("[INFO] Detected buoy(s):")
    for b in buoy_detections:
        side = "LEFT" if b['x'] + b['w'] / 2 < frame_dims['width'] / 2 else "RIGHT"
        print(f" - Type: {b['type']}, Side: {side}")

    if len(buoy_detections) == 1:
        b = buoy_detections[0]
        ...
        if (left_type, right_type) in valid_corridors:
            print("[DECISION] Valid corridor detected. Aligning to the midpoint between buoys.")
            heading_adjustment = math.degrees(math.atan2(midpoint_offset_cm, distance_cm))
        else:
            print("[DECISION] Invalid corridor. Applying avoidance logic based on left buoy.")
    ...
    correction_direction = (
        "LEFT" if heading_adjustment < 0
        else "RIGHT" if heading_adjustment > 0
        else "TOWARD WAYPOINT"
    )
    print(f"[INFO] Heading correction: {correction_direction} ({heading_adjustment:.2f}°)")
